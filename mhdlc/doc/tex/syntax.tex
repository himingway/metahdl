...
\begin{lstlisting}[caption={MetaHDL FF syntax}]
...
\hfill
\begin{minipage}[t]{.48\textwidth}
\begin{lstlisting}[caption={Generated SystemVerilog}]
...
name \mhdl{clk} and \mhdl{rst\_n} will be used. State transition is explicitly
...
\begin{figure}[p]
\begin{minipage}[t]{.48\textwidth}
\begin{lstlisting}[caption={FSM in MetaHDL}, label={lst:fsm in mhdl}]
fsm cmdrx, clk, rst_n;
  cm_pim_ack = 1'b0;
...
\hfill
\begin{minipage}[t]{.48\textwidth}
\begin{lstlisting}[caption={Generated FSM in SystemVerilog}, label={lst:fsm in sv}]
// other declarations...
...
\begin{figure}[p]
  \begin{minipage}[t]{.48\textwidth}
    \begin{lstlisting}[caption={\texttt{moda.mhdl}}]
input i1;
input i2;
output o1;
output [1:0] o2;
    \end{lstlisting}
    \begin{lstlisting}[caption={Instantiation in MetaHDL}]
...
\begin{figure}[p]
  \begin{minipage}[t]{.48\textwidth}
    \begin{lstlisting}[caption={Inner module: \texttt{modc} in \gls{mhdl}}, label={lst:modc in mhdl}]
parameter A = 4;
parameter B = 5; 
parameter C = A + B;
...
assign o1[C-1:0] = {~i1[A-1:0], i2[B-1:0]};
    \end{lstlisting}
    \begin{lstlisting}[caption={Generated \texttt{modc} in SystemVerilog}, label={lst:modc in sv}]
module modc (
  i1, 
  i2, 
  o1);
...
assign o1[C - 1:0] = {~i1[A - 1:0], i2[B - 1:0]};
    ...
    \end{lstlisting}
    \begin{lstlisting}[caption={Wrapper Module in MetaHDL}, label={lst:wrapper in mhdl}]
parameter SETA = 8,
          SETB = 9;
...
modc #( .A(SETA) ) x2_modc (x2_ +,
  ...
  \hfill
  \begin{minipage}[t]{.48\textwidth}
    \begin{lstlisting}[caption={Generated wrapper module in SystemVerilog}, label={lst:wrapper in sv}]
module modwrapper (
  x0_i1, 
  x0_i2, 
  x0_o1, 
